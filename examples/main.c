...
static void print_usage(const char* program) {
    fprintf(stderr,
            "Usage:\n"
            "  %s <masterIndex> <op> <slaveId> <busId> <slot> <motorId> <model> "
            "[--eff] [--canfd] [--flag <value>] [args...]\n"
            "\n"
            "masterIndex selects /dev/EtherCAT<masterIndex>, for example 0.\n"
            ...
            "\n"
            "Example:\n"
            "  sudo %s 0 spd 0 2 0 11 2 1.0 2.0 1\n",
            program, program);
}
...
int main(int argc, char** argv) {
    ...
    const char* master_index = argv[1];
    ...
    EcMaster master;
    if (!ec_master_open(&master, master_index)) {
        return 1;
    }
